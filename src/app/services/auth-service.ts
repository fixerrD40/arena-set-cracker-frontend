...
export class AuthService {
  ...
  register(credentials: { username: string; password: string }): Observable<string> {
    return this.http.post(this.registrationUrl, credentials, { responseType: 'text' }).pipe(
  ...
  login(credentials: { username: string; password: string }): Observable<string> {
    return this.http.post(this.authenticationUrl, credentials, { responseType: 'text' }).pipe(
      tap(token => this.saveToken(token)),
      catchError(this.handleError)
    );
  }
